...
class BackupForGKEClient(metaclass=BackupForGKEClientMeta):
    ...
    @staticmethod
    def backup_path(project: str,location: str,backup_plan: str,backup: str,) -> str:
        """Returns a fully-qualified backup string."""
        return "projects/{project}/locations/{location}/backupPlans/{backup_plan}/backups/{backup}".format(project=project, location=location, backup_plan=backup_plan, backup=backup, )

    @staticmethod
    def parse_backup_path(path: str) -> Dict[str,str]:
        """Parses a backup path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/backupPlans/(?P<backup_plan>.+?)/backups/(?P<backup>.+?)$", path)
    ...
    @staticmethod
    def restore_path(project: str,location: str,restore_plan: str,restore: str,) -> str:
        """Returns a fully-qualified restore string."""
        return "projects/{project}/locations/{location}/restorePlans/{restore_plan}/restores/{restore}".format(project=project, location=location, restore_plan=restore_plan, restore=restore, )

    @staticmethod
    def parse_restore_path(path: str) -> Dict[str,str]:
        """Parses a restore path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/restorePlans/(?P<restore_plan>.+?)/restores/(?P<restore>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def restore_plan_path(project: str,location: str,restore_plan: str,) -> str:
        """Returns a fully-qualified restore_plan string."""
        return "projects/{project}/locations/{location}/restorePlans/{restore_plan}".format(project=project, location=location, restore_plan=restore_plan, )

    @staticmethod
    def parse_restore_plan_path(path: str) -> Dict[str,str]:
        """Parses a restore_plan path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/restorePlans/(?P<restore_plan>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def volume_backup_path(project: str,location: str,backup_plan: str,backup: str,volume_backup: str,) -> str:
        """Returns a fully-qualified volume_backup string."""
        return "projects/{project}/locations/{location}/backupPlans/{backup_plan}/backups/{backup}/volumeBackups/{volume_backup}".format(project=project, location=location, backup_plan=backup_plan, backup=backup, volume_backup=volume_backup, )

    @staticmethod
    def parse_volume_backup_path(path: str) -> Dict[str,str]:
        """Parses a volume_backup path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/backupPlans/(?P<backup_plan>.+?)/backups/(?P<backup>.+?)/volumeBackups/(?P<volume_backup>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def volume_restore_path(project: str,location: str,restore_plan: str,restore: str,volume_restore: str,) -> str:
        """Returns a fully-qualified volume_restore string."""
        return "projects/{project}/locations/{location}/restorePlans/{restore_plan}/restores/{restore}/volumeRestores/{volume_restore}".format(project=project, location=location, restore_plan=restore_plan, restore=restore, volume_restore=volume_restore, )

    @staticmethod
    def parse_volume_restore_path(path: str) -> Dict[str,str]:
        """Parses a volume_restore path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/restorePlans/(?P<restore_plan>.+?)/restores/(?P<restore>.+?)/volumeRestores/(?P<volume_restore>.+?)$", path)
        return m.groupdict() if m else {}
